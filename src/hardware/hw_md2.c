...
boolean HWR_DrawModel(gl_vissprite_t *spr)
{
	...
	if (spr->mobj->subsector)
	{
		...
		if (sector->numlights)
		{
			...
			light = P_GetSectorLightNumAt(sector, spr->mobj->x, spr->mobj->y, spr->mobj->z + spr->mobj->height); // Always use the light at the top instead of whatever I was doing before
