...
void V_DrawFillConsoleMap(INT32 x, INT32 y, INT32 w, INT32 h, INT32 c)
{
	...
	if (rendermode == render_opengl)
	{
		...
		HWR_DrawConsoleFill(x, y, w, h, c, hwcolor, alpha);	// we still use the regular color stuff but only for flags. actual draw color is "hwcolor" for this.
		return;
	}
#endif
...
void V_DrawFadeConsBack(INT32 plines)
{
	...
	if (rendermode == render_opengl)
	{
		UINT32 hwcolor = V_GetHWConsBackColor();
		UINT8 alpha = (cons_backcolor.value == 1) ? 0xC0 : 0x80; // A hack to make black darker like in software
		HWR_DrawConsoleBack(hwcolor, alpha, plines);
		return;
	}
#endif
