...
UINT32 cpu_feature_flags;

static void I_SetCPUFeatureFlags(void)
{
#if defined (HAVE_SDLCPUINFO)
	if (SDL_HasSSE())
		cpu_feature_flags |= CPU_FEATURE_SSE;
	if (SDL_HasAVX())
		cpu_feature_flags |= CPU_FEATURE_AVX;
#elif defined(__x86_64__)
	// all 64-bit x86 machines have SSE, so this is a safe assumption to make
	cpu_feature_flags = CPU_FEATURE_SSE;
#endif
	CONS_Printf("CPU features detected: ");
	if (cpu_feature_flags & CPU_FEATURE_SSE)
		CONS_Printf("SSE,");
	if (cpu_feature_flags & CPU_FEATURE_AVX)
		CONS_Printf("AVX,");
	CONS_Printf("\n");
}
