...
{
	SDL_Event evt;
	char* dropped_filedir, * dropped_text; //allow drag and drop addons and text
	...
	while (SDL_PollEvent(&evt))
	{
		...
		case SDL_DROPFILE:
			...
			if (pathisdirectory(dropped_filedir))
				COM_BufInsertText(va("addfolder \"%s\"", dropped_filedir));
			else
				COM_BufInsertText(va("addfile \"%s\"", dropped_filedir));
			SDL_free(dropped_filedir);    // Free dropped_filedir memory
			break;
		case SDL_DROPTEXT:
			dropped_text = evt.drop.file;
			CON_InputAddString(va("%s\n", dropped_text));
			SDL_free(dropped_text);
			break;
		}
	}
